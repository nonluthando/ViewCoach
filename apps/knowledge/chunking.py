...
def _overlap_tail(value, overlap_characters):
    if overlap_characters <= 0 or len(value) <= overlap_characters:
        return value if overlap_characters > 0 else ""
...
def chunk_markdown(
    ...
    *,
    ...
    def flush():
        ...
        overlap = _overlap_tail(content, overlap_characters)
        current_parts = [overlap] if overlap else []

    for heading, paragraph in prepared_paragraphs:
        proposed_parts = [*current_parts, paragraph]
