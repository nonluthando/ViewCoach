...
class Command(BaseCommand):
    help = "Run the deterministic ViewCoach retrieval evaluation set."
    ...
    def handle(self, *args, **options):
        ...
        for case in RETRIEVAL_EVALUATION_CASES:
            ...
            self.stdout.write(f"{outcome} {case.name}: retrieved={list(retrieved_slugs)}")

        total = len(RETRIEVAL_EVALUATION_CASES)
        score = passed / total if total else 0
        self.stdout.write(f"Retrieval score: {passed}/{total} ({score:.0%})")
        if score < minimum_score:
            raise CommandError("Retrieval evaluation did not meet the minimum score.")
