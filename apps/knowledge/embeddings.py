...
def _normalise_vector(values, *, dimensions):
...
class GeminiEmbeddingProvider:
    ...
    def embed_documents(self, *, texts, titles=None):
        ...
        if not cleaned or any(not text for text in cleaned):
            raise ValueError(
                "Document embedding input must contain "
                "non-empty text."
            )

        document_titles = list(
            titles or [None] * len(cleaned)
        )
        if len(document_titles) != len(cleaned):
            raise ValueError(
                "Each document embedding must have a matching title."
            )
        ...
        for text, title in zip(
            ...
            embeddings.append(
                self._extract_embedding(response)
            )
        return embeddings
    ...
    def embed_query(self, query):
        ...
        if not cleaned_query:
            raise ValueError(
                "Query embedding input cannot be empty."
            )
